...
namespace Marvin255\Bxrequest\installer;
...
class Composer
{
     ...
     * @var string
     */
    protected static $module = 'bxrequest';
    ...
    protected static function getInstallingFoldersPathes(Event $event)
    {
        ...
        $phpPsrHttpMessageFolder = self::getLibraryFolder($event, 'psr', 'http-message');
        $phpPsrHttpMessageFolder .= '/src';

        return [
            $libFolder => $modulesFolder . '/' . self::$vendor . '.' . self::$module,
            $phpPsrHttpMessageFolder => $modulesFolder . '/' . self::$vendor . '.' . self::$module . '/psr_http_message',
        ];
    }
